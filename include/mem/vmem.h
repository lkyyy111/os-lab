...
VA和PA的构成:
VA: VP_N[2] + VP_N[1] + VP_N[0] + offset    9 + 9 + 9 + 12 = 39 (使用uint64存储) => 最大虚拟地址为512GB 
PA: PPN[2] + PPN[1] + PPN[0] + offset   26 + 9 + 9 + 12 = 56 (使用uint64存储)

为什么是 "9" : 4KB / uint64 = 512 = 2^9 所以一个物理页可以存放512个页表项
我们使用三级页表对应三级VP_N, VP_N[2]称为顶级页表、VP_N[1]称为次级页表、VP_N[0]称为低级页表
...
// 获取虚拟地址中的虚拟页(VP_N)信息 占9bit
#define VA_SHIFT(level)         (12 + 9 * (level))
#define VA_TO_VP_N(va,level)     ((((uint64)(va)) >> VA_SHIFT(level)) & 0x1FF)
